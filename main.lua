...
--
-- The content editor (tools/content-editor/) authors mod folders for maps,
-- Pokemon and items: `love . --content-editor` (optional `--mod mods/id`).

local editorMode = os.getenv("POKEPORT_EDITOR") == "1" or POKEPORT_EDITOR_MODE == true
local contentEditorMode = os.getenv("POKEPORT_CONTENT_EDITOR") == "1"
...
-- Content editor App.lua shadows the save editor's; Kit/Theme still resolve
-- from tools/save-editor on the same path.
local function addContentEditorRequirePath()
  local fs = love.filesystem
  if not (fs.setRequirePath and fs.getRequirePath) then
    package.path = fs.getSource() .. "/tools/content-editor/?.lua;"
                .. fs.getSource() .. "/tools/content-editor/panels/?.lua;"
                .. fs.getSource() .. "/tools/save-editor/?.lua;"
                .. package.path
    return
  end
  local current = fs.getRequirePath()
  if current:find("tools/content%-editor") then return end
  fs.setRequirePath(
...
function love.load(args)
  ...
  for i, a in ipairs(args or {}) do
    if a == "--editor" then
      editorMode = true
    elseif a == "--content-editor" then
      contentEditorMode = true
      editorMode = true
    elseif a == "--developer" then
      _G.POKEPORT_DEV_MODE = true
    elseif a == "--save" and args[i + 1] and args[i + 1] ~= "" then
      savePath = args[i + 1]
    elseif a == "--mod" and args[i + 1] and args[i + 1] ~= "" then
      modPath = args[i + 1]
  ...
  if contentEditorMode then
    ...
    require("src.import.CacheFs").mountVersion(version)
    addContentEditorRequirePath()
    EditorApp = require("App")
    EditorApp.load(modPath, { version = version })
    return
  end
